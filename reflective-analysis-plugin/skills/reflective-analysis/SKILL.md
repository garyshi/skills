...
**Popularity is a memetic filter, not a truth filter.** Beyond what any source *intends*, the aggregate is distorted by how ideas spread — no bad actor required. A view becomes prevalent partly because it is catchy, simple, and easy to repeat, and catchiness usually comes from over-simplifying; so the most-repeated version of an idea is often a slogan that flattened away the load-bearing nuance, not the most correct version. Conversely, an insight that is correct but early, unfashionable, or counter to the current narrative tends to be quiet — scattered, hedged, in low-visibility places, or waved off as fringe — and is therefore under-represented in anything volume-weighted, sometimes to the point of going unheard. Reliably *discovering* an ahead-of-its-time insight is too much to ask; but hold the idea in mind and avoid the two cheap errors it causes: (1) do **not** treat how *often* a view appears as evidence for it — weigh the argument and evidence, and read a suspiciously clean, popular framing as a cue to go find the nuance it dropped; (2) do **not** treat rarity or unfashionableness as evidence against a view — when a minority or early view has a real mechanism and evidence behind it, give it a fair hearing instead of discounting it for being quiet.
