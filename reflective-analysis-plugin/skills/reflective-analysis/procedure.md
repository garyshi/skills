...
## Decompose to the ground level — invariant vs. contingent limits

Push the unit of analysis past the label, down to the concrete activity: not "law," not even "contract review," but *what a lawyer actually does, step by step, when reviewing a contract*. Two payoffs:

* **It shows how much is already doable.** At the ground level a profession's work is a bundle of concrete sub-activities, and AI can often already do a large chunk of them. The abstraction ("can AI be a lawyer / assess an investment / treat a tricky case?") hides this; the decomposition ("extract the obligations, flag deviations from precedent, draft the redline?") exposes it. The honest finding is usually "AI does most of the sub-activities, not 100%, and cannot hold the accountable role" — very different from "AI can't do this."
* **It separates genuine invariants from contingent current-state limits.** For each remaining limit, ask: is it a logical/physical invariant, or a constraint of the *current* state — capability, tooling, cost, trust, authority, liability, regulation — that could change as conditions shift? (Illustration: "AI can't sign off on the audit / be the physician of record" is not a law of nature; it reflects today's capability and today's allocation of authority, and could shift.) Mark such limits **contingent and movable**, note what would move them, and hand them to the obstacle-to-opportunity pass. Do not silently promote a contingent limit to a permanent one — that is how an analysis quietly encodes the status quo as a law. (Identifying these contingent limits is part of the analysis; *assessing how much of each is addressable with focused near-term effort* is the optional deeper ground-level pass, run only on request — see `SKILL.md` → "The deeper pass".)
