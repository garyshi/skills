...
* **It separates genuine invariants from contingent current-state limits.** For each remaining limit, ask: is it a logical/physical invariant, or a constraint of the *current* state — capability, trust, authority, liability, regulation — that could move as capability grows? "AI can't sign off on the audit / be the physician of record" is not a law of nature; it reflects today's capability and today's allocation of authority, and could shift. Mark such limits **contingent and movable**, note what would move them, and hand them to the obstacle-to-opportunity pass. Do not silently promote a contingent limit to a permanent one — that is how an analysis quietly encodes the status quo as a law. (Identifying these contingent limits is part of the present-state picture; *assessing how much of each is solvable with focused near-term work* is Rung 2 of the speculation ladder, run only on request — see `SKILL.md` → "Speculation depth".)
